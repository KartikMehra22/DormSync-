...
function EditAttendanceModal({ record, onClose, onSuccess }) {
    ...
    const [checkIn, setCheckIn] = useState(record.checkInTime ? new Date(record.checkInTime).toTimeString().slice(0, 5) : "");
    const [checkOut, setCheckOut] = useState(record.checkOutTime ? new Date(record.checkOutTime).toTimeString().slice(0, 5) : "");
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            const checkInTime = checkIn ? new Date(`${dateBase}T${checkIn}`).toISOString() : null;
            const checkOutTime = checkOut ? new Date(`${dateBase}T${checkOut}`).toISOString() : null;

            await api.attendance.update(token, record.id, {
                status,
                checkInTime,
                checkOutTime,
                remarks
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6">
                ...
                <p className="text-sm text-gray-600 mb-4">For {record.user.name}</p>
                <form onSubmit={handleSave} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm text-gray-700 mb-1">Entry Time</label>
                            <input type="time" className="w-full border p-2 rounded" value={checkIn} onChange={e => setCheckIn(e.target.value)} />
                        </div>
                        <div>
                            <label className="block text-sm text-gray-700 mb-1">Exit Time</label>
                            <input type="time" className="w-full border p-2 rounded" value={checkOut} onChange={e => setCheckOut(e.target.value)} />
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm text-gray-700 mb-1">Remarks</label>
                        <textarea className="w-full border p-2 rounded" rows="2" value={remarks} onChange={e => setRemarks(e.target.value)}></textarea>
                    </div>
                    <div className="flex gap-2 justify-end pt-2">
                        <button type="button" onClick={onClose} className="px-4 py-2 border rounded">Cancel</button>
                        <button type="submit" disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded">Save</button>
                    </div>
                </form>
            </div>
        </div>
